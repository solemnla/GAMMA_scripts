...
process_probe <- function(probe_id) {
  ...
  gene_name <- unique(QTL_clumped_tmp$gene_name)
  nsnps=probe_SNP_nums$Freq[which(probe_SNP_nums$probeID == probe_id)]
...
res_final=res[,c("qtl_name","gene_name","exposure","method","bxy","se","p","lower","upper","nsnps")]
colnames(res_final)=c("qtl_name","gene_name","probeID","method","bxy","se","pval","bxy_lower","bxy_upper","nsnps")

write.table(res_final,paste0(OUTPUT,"/MR/GSMR/detail/",trait_name,"_",qtl_name,"_",process_num,"_MR_comparison.txt"),
